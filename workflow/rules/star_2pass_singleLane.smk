...
rule htseq:
        input:
            bam = 'results/star/{sample}_p2/aligned.sortedByCoord.out.bam',
            gtf = 'resources/genome.gtf'
        output:
            union = '{sample}_HTSeq_union_gtf_no_gene_ID.log',
            csv = '{sample}_HTSeq.csv'
        threads: 1
        shell:
            'htseq-count -m union -s no -t gene -i ID -r pos -f bam {input.bam} {input.gtf} &> {output.union} && '
